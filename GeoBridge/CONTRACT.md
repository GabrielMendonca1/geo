...
| `GEO_AGENT_INTERRUPT_KEY` | `Escape` | tmux key name sent by `/term/agent-interrupt` (see the rationale there before changing it to `C-c`) |
...
- **Mac path**: herdr exposes no "waiting for input" signal, so the answer is always `asking:false, kind:"", options:[]` after a successful resolution. **This is honesty, not a stub**: nothing is invented, and the client must not read it as "the Mac agent is free". A failed scan is still `503`, never `asking:false`.
...
Answers the question `/term/agent-ask` detected. **VM only** (`?session=` required; absent, malformed, or mixed with `project`/`pane` → `400 {"error":"bad_target"}`) — the Mac path has nothing to answer, since `/term/agent-ask` never reports a question there.
...
- Codes: `200 {"ok":true,"index":<n>}` · `400 bad_target` / `invalid_body` / `bad_option` · `404 no_session` / `404 no_agent` · `409 {"error":"not_asking"}` when the fresh capture shows no question (nothing is written) · `409 {"error":"no_attach"}` if the pty write itself failed · `503 {"error":"unavailable"}` when the lookup/capture blew up.

### POST /term/agent-interrupt

Interrupts the agent. No body.

- `?session=` **required** (VM only), same validation → `400 {"error":"bad_target"}`.
...
- **`200 {"ok":true}` means the key was delivered to tmux, not that the agent stopped.** There is no acknowledgement from a TUI; the client confirms by polling (`/term/agent-chat`, `/term/agent-work`).
- Codes: `200` · `400 bad_target` · `404 {"error":"no_session"}` · `404 {"error":"no_agent"}` · `503 {"error":"unavailable"}` when tmux is missing/timed out or `send-keys` failed.
