...
public struct ParsedQuickAdd: Equatable {
    ...
    public var kind: TaskKind { body.kind }
}

public enum QuickAddParser {
    public static func parse(_ input: String, now: Date = Date(), calendar: Calendar = .current) -> ParsedQuickAdd {
        ...
        if let timeMatch = extractAbsoluteTime(from: trimmed, ranges: &ranges, now: now, calendar: calendar) {
            ...
            return ParsedQuickAdd(
                title: title,
                body: .event(start: timeMatch.date, end: end, externalEKEventID: nil),
                confidence: .high
            )
        }
...
private extension QuickAddParser {
    static func extractRecurrence(
        ...
        if let m = firstMatch(in: input, pattern: "\\btwice\\s+daily\\b"),
           let r = range(of: m, in: input) {
            ranges.append(r)
            let timeOfDay = noExplicitTime(now: now, calendar: calendar)
        ...
        if let m = firstMatch(in: input, pattern: "\\bevery\\s+weekday\\b"),
           let r = range(of: m, in: input) {
            ranges.append(r)
            let timeOfDay = resolvedTimeOfDay(default: noExplicitTime(now: now, calendar: calendar), from: input, ranges: &ranges, now: now, calendar: calendar)
        ...
        if let m = firstMatch(in: input, pattern: dailyPattern),
           let r = range(of: m, in: input) {
            ...
            if matched.contains("morning") {
                ...
                timeOfDay = noExplicitTime(now: now, calendar: calendar)
            }
            let resolved = resolvedTimeOfDay(default: timeOfDay, from: input, ranges: &ranges, now: now, calendar: calendar)
            return RecurrenceMatch(rule: .daily, timeOfDay: resolved, confidence: .high)
        }

        if let m = firstMatch(in: input, pattern: "\\b(weekly|every\\s+week|each\\s+week)\\b"),
           let r = range(of: m, in: input) {
            ranges.append(r)
            let defaultTimeOfDay = noExplicitTime(now: now, calendar: calendar)
            ...
            let timeOfDay = resolvedTimeOfDay(default: defaultTimeOfDay, from: input, ranges: &ranges, now: now, calendar: calendar)
    ...
    static func resolvedTimeOfDay(
        ...
        if let timeMatch = extractAbsoluteTime(from: input, ranges: &ranges, now: now, calendar: calendar, restrictToTime: true) {
            return timeMatch.date
        }
        return fallback
    }

    // Hábito sem hora de relógio explícita → sem horário (all-day). Evita o "horário aleatório"
    // que a hora-corrente da digitação produzia.
    static func noExplicitTime(now: Date, calendar: Calendar) -> Date {
