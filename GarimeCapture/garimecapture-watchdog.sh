...
if [ -f "$STATUS_FILE" ]; then
  ...
  if [ "$spool_files" -gt 0 ] && [ "$stall" -gt "$UPLOAD_STALL_MAX" ]; then
    log "ERROR: upload not progressing — ${spool_files} file(s) spooled (oldest ${spool_oldest}s), no successful upload in ${stall}s, ${failures} consecutive failure(s)"
    exit 2
  fi
  if [ "$failures" -ge "$UPLOAD_FAIL_ALERT" ]; then
    log "ERROR: uploader failing — ${failures} consecutive failure(s), ${spool_files} file(s) spooled (oldest ${spool_oldest}s)"
    exit 2
  fi
fi
