...
grep -q 'NSGlassEffectView' "$ROOT/Sources/HubPanel.swift"
check $? "the card uses the real Liquid Glass view, not frosted vibrancy"
grep -q 'if #available(macOS 26.0' "$ROOT/Sources/HubPanel.swift"
check $? "Liquid Glass is behind an availability guard"
grep -q 'material = .popover' "$ROOT/Sources/HubPanel.swift"
check $? "older macOS still falls back to the popover material"
grep -q 'HubHairline' "$ROOT/Sources/HubPanel.swift"
check "$([ $? -ne 0 ] && echo 0 || echo 1)" "no hand-drawn border: native popovers have none"
grep -q 'blendingMode = .behindWindow' "$ROOT/Sources/HubPanel.swift"
check $? "the glass blurs what is behind the window"
