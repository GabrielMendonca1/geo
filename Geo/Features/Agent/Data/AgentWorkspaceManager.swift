...
actor AIWorkspaceManager {
    ...
    private func dispatch(
        ...
        do {
            ...
            guard detectedPi.isAvailable, let piPath = detectedPi.executablePath else {
                throw dispatchError(detectedPi.statusDetail ?? "pi runtime not available.")
            }
            ...
            guard attemptNumber <= maxTurns else {
                ...
                let message = "Skipped dispatch for \(issue.identifier): would exceed max_turns (\(maxTurns))."
            ...
            let sessionID = "pi-\(UUID().uuidString.prefix(8))"
            ...
            let modelArgument = combinedModelArgument(model: effectiveModel, effort: effectiveEffort)
            let resumeSessionID = issue.symphonySessionID?.trimmingCharacters(in: .whitespacesAndNewlines)
            attempts.insert(AIRunAttempt(
                id: agentAttemptID,
                issueID: issue.id,
                issueIdentifier: issue.identifier,
            ...
            liveSessions[sessionID] = AILiveSession(
                ...
                turnID: "background",
                processID: nil,
                lastEvent: "pi launching",
                lastTimestamp: Date(),
                lastMessage: "pi is starting for \(project.name).",
                inputTokens: 0,
                outputTokens: 0,
                totalTokens: 0,
                turnCount: 0,
                ...
                piSessionID: (resumeSessionID?.isEmpty == false) ? resumeSessionID : nil
            )
            do {
                let terminationHandler: @Sendable (Int32) -> Void = { [weak self] status in
                    Task {
                        await self?.handleAgentProcessTermination(
                            sessionID: sessionID,
                            terminationStatus: status
                        )
                    }
                }
                let agentProcess = try launchPiProcess(
                    binaryPath: piPath,
                    workingDirectory: preparedAgent.workingDirectoryURL,
                    prompt: preparedAgent.prompt,
                    agentRootURL: preparedAgent.rootURL,
                    provider: provider,
                    modelArgument: modelArgument,
                    resumeSessionID: (resumeSessionID?.isEmpty == false) ? resumeSessionID : nil,
                    issueID: issue.id,
                    sessionID: sessionID,
                    onTermination: terminationHandler
                )
                guard liveSessions[sessionID] != nil else { return }
                if let index = attempts.firstIndex(where: { $0.id == agentAttemptID }) {
                    attempts[index].status = .running
                }
                liveSessions[sessionID]?.lastEvent = "pi launched"
                liveSessions[sessionID]?.lastTimestamp = Date()
                liveSessions[sessionID]?.lastMessage = "pi launched for \(project.name)."
                liveSessions[sessionID]?.processID = String(agentProcess.processIdentifier)
                runningProcesses[sessionID] = agentProcess
                if !agentProcess.isRunning {
                    await handleAgentProcessTermination(
                        sessionID: sessionID,
                        terminationStatus: agentProcess.terminationStatus
                    )
                    return
                }
                ...
                if let index = attempts.firstIndex(where: { $0.id == agentAttemptID }) {
                    ...
                    attempts[index].error = error.localizedDescription
                    attempts[index].finishedAt = Date()
                }
                let message = "pi launch failed for \(issue.identifier): \(error.localizedDescription)"
                appendLog(.error, message)
                throw dispatchError(message)
            }
            appendLog(.info, "Dispatched \(issue.identifier) to pi (\(provider)) for \(project.name) in \(workspace.path).")
        } catch {
            claimedIssueIDs.remove(issue.id)
    ...
    private func handleAgentProcessTermination(sessionID: String, terminationStatus: Int32) async {
        runningProcesses.removeValue(forKey: sessionID)
        guard let session = liveSessions[sessionID] else { return }
        let workflow = await currentWorkflow()
        do {
            try runHook(
                workflow.config.hooks.afterRun,
                cwd: URL(fileURLWithPath: session.workspacePath, isDirectory: true),
                timeoutMS: workflow.config.hooks.timeoutMS
            )
        } catch {
            appendLog(.warning, "after_run failed for \(session.issueIdentifier): \(error.localizedDescription)")
        }

        let metadataURL = URL(fileURLWithPath: session.workspacePath, isDirectory: true)
        ...
        if terminationStatus == 0 {
            ...
            let activeStates = Set(workflow.config.tracker.activeStates.map(normalizeState))
            let terminalStates = Set(workflow.config.tracker.terminalStates.map(normalizeState))
            let refreshed = try? await fetchIssueStatesByIDs([session.issueID], workflow: workflow).first
            ...
            let attemptCount = attempts.filter { $0.issueID == session.issueID }.count
            let maxTurns = max(workflow.config.agent.maxTurns, 1)
            liveSessions.removeValue(forKey: sessionID)
            claimedIssueIDs.remove(session.issueID)
            if let normalizedState, terminalStates.contains(normalizedState) {
                completedIssueIDs.insert(session.issueID)
                markAttempt(issueID: session.issueID, status: .succeeded, error: nil)
                cleanupWorkspace(issueID: session.issueID)
                appendLog(.info, "pi finished \(session.issueIdentifier); tracker state is terminal.")
                return
            }
            if !stillActive {
                markAttempt(issueID: session.issueID, status: .succeeded, error: nil)
                appendLog(.info, "pi finished \(session.issueIdentifier); tracker state is no longer active.")
                return
            }
            if stillActive && attemptCount < maxTurns {
                ...
                appendLog(
                    .info,
                    "pi finished turn \(attemptCount) for \(session.issueIdentifier); scheduling continuation (next: \(attemptCount + 1)/\(maxTurns))."
                )
                let capturedIssueID = session.issueID
                Task { [weak self] in
                    guard let wf = await self?.currentWorkflow() else { return }
                    await self?.scheduleRetry(issueID: capturedIssueID, workflow: wf, abnormal: false)
                }
                return
            }
            if stillActive && attemptCount >= maxTurns {
                let outputPath = metadataURL.appendingPathComponent("output.log").path
                let message = "pi reached max_turns (\(maxTurns)) for \(session.issueIdentifier). See \(outputPath)."
                markAttempt(issueID: session.issueID, status: .failed, error: message)
                appendLog(.warning, message)
                return
            }
        }

        let reportURL = metadataURL.appendingPathComponent("report.md")
        if let report = try? String(contentsOf: reportURL, encoding: .utf8),
           !report.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty {
            await ingestAgentReports()
            liveSessions.removeValue(forKey: sessionID)
            claimedIssueIDs.remove(session.issueID)
            appendLog(.info, "pi exited with status \(terminationStatus) but left a report for \(session.issueIdentifier); ingested.")
            return
        }
        ...
        claimedIssueIDs.remove(session.issueID)
        let outputPath = metadataURL.appendingPathComponent("output.log").path
        let message = terminationStatus == 0
            ? "pi exited without writing a report. See \(outputPath)."
            : "pi exited with status \(terminationStatus). See \(outputPath)."
        markAttempt(issueID: session.issueID, status: .failed, error: message)
        appendLog(.error, message)
        let capturedIssueID = session.issueID
        let abnormal = terminationStatus != 0
        Task { [weak self] in
            guard let wf = await self?.currentWorkflow() else { return }
            await self?.scheduleRetry(issueID: capturedIssueID, workflow: wf, abnormal: abnormal)
        }
    }
    ...
    private func launchPiProcess(
        ...
        let systemPrompt = piSystemPrompt(reportPath: reportPath(forAgentWorkspacePath: agentRootURL.path))
        ...
        if let modelArgument {
            components.append(contentsOf: ["--model", shellQuote(modelArgument)])
        }
        if let resumeSessionID, !resumeSessionID.isEmpty {
            components.append(contentsOf: ["--session", shellQuote(resumeSessionID)])
        }
        let commandLine = components.joined(separator: " ")

        let metaURL = agentRootURL.appendingPathComponent(".symphony", isDirectory: true)
        try FileManager.default.createDirectory(at: metaURL, withIntermediateDirectories: true)
        let logURL = metaURL.appendingPathComponent("output.log")
        ...
        process.environment = backgroundAgentEnvironment()
        ...
        pipe.fileHandleForReading.readabilityHandler = { [weak self] handle in
            ...
            Task { [weak self] in
                for line in lines {
                    await self?.applyPiStreamLine(sessionID: sessionID, issueID: issueID, line: line)
                }
            }
        }
    ...
    private func applyPiStreamLine(sessionID: String, issueID: String, line: String) async {
        let trimmed = line.trimmingCharacters(in: .whitespacesAndNewlines)
        guard !trimmed.isEmpty else { return }
        guard let data = trimmed.data(using: .utf8),
              let json = try? JSONSerialization.jsonObject(with: data) as? [String: Any],
              let type = json["type"] as? String else {
            aiLogger.debug("pi non-JSON line: \(String(trimmed.prefix(160)), privacy: .public)")
            return
        }
        guard var session = liveSessions[sessionID] else { return }
        session.lastTimestamp = Date()

        switch type {
        case "session":
            if let id = json["id"] as? String, session.piSessionID != id {
        ...
        case "agent_start":
            session.lastEvent = "agent_start"
        case "turn_start":
            session.lastEvent = "turn_start"
        case "message_start":
            session.lastEvent = "message_start"
        case "message_update":
            if let event = json["assistantMessageEvent"] as? [String: Any],
               let kind = event["type"] as? String {
                if kind == "text_delta", let delta = event["delta"] as? String {
                    let combined = (session.lastMessage ?? "") + delta
                    let trimmedCombined = combined
                        .replacingOccurrences(of: "\n", with: " ")
                        .trimmingCharacters(in: .whitespacesAndNewlines)
                    session.lastMessage = String(trimmedCombined.suffix(160))
                } else {
                    session.lastEvent = kind
                }
            }
        case "message_end":
            session.lastEvent = "message_end"
        case "tool_execution_start":
            if let toolName = json["toolName"] as? String {
                let call = AIToolCall(
                    ...
                    name: toolName,
                    target: toolTarget(args: json["args"] as? [String: Any], toolName: toolName),
                    timestamp: Date()
                )
                session.recentToolCalls.append(call)
                if session.recentToolCalls.count > 50 {
                    session.recentToolCalls.removeFirst(session.recentToolCalls.count - 50)
                }
                session.lastEvent = "tool:\(toolName)"
            }
        case "tool_execution_end":
            if let toolName = json["toolName"] as? String {
                if (json["isError"] as? Bool) == true {
                    session.lastEvent = "tool error:\(toolName)"
                }
            }
        case "turn_end":
            if let message = json["message"] as? [String: Any],
               let usage = message["usage"] as? [String: Any] {
                ...
                let total = usage["totalTokens"] as? Int ?? (input + output)
                let inputDelta = max(input - session.inputTokens, 0)
                let outputDelta = max(output - session.outputTokens, 0)
                let totalDelta = max(total - session.totalTokens, 0)
                agentTotals.inputTokens += inputDelta
                agentTotals.outputTokens += outputDelta
                agentTotals.totalTokens += totalDelta
                session.inputTokens = input
                session.outputTokens = output
                session.totalTokens = total
            }
            session.turnCount += 1
            session.lastEvent = "turn_end"
        case "agent_end":
            session.lastEvent = "agent_end"
            if let messages = json["messages"] as? [[String: Any]],
               let last = messages.last,
               let stopReason = last["stopReason"] as? String {
                let attemptStatus: AIRunStatus
                ...
                case "length":
                    ...
                    errorMessage = "context length limit"
                case "aborted":
                    attemptStatus = .canceled
                case "error":
                    attemptStatus = .failed
                    errorMessage = (last["errorMessage"] as? String) ?? "agent error"
                default:
                    attemptStatus = .succeeded
                }
                markAttempt(issueID: session.issueID, status: attemptStatus, error: errorMessage)
            }
        case "queue_update":
            session.lastEvent = "queue_update"
        case "compaction_start":
            session.lastEvent = "compacting"
        case "compaction_end":
            session.lastEvent = "compaction_end"
        case "auto_retry_start":
            let attempt = json["attempt"] as? Int ?? 0
            let max = json["maxAttempts"] as? Int ?? 0
            session.lastEvent = "retry \(attempt)/\(max)"
        case "auto_retry_end":
            session.lastEvent = "auto_retry_end"
        default:
            aiLogger.debug("pi unknown event: \(type, privacy: .public)")
        }

        liveSessions[sessionID] = session
    }

    nonisolated private func toolKind(for name: String) -> AIToolCall.Kind {
        switch name {
        case "Read": return .read
        case "Edit", "MultiEdit": return .edit
        case "Write": return .write
        case "Bash", "Shell": return .run
        case "Grep", "Glob", "Search": return .search
        default: return .other
        }
    }

    nonisolated private func toolTarget(args: [String: Any]?, toolName: String) -> String? {
        guard let args else { return nil }
        if let path = args["file_path"] as? String { return path }
        if let path = args["path"] as? String { return path }
        if let pattern = args["pattern"] as? String { return pattern }
        if let command = args["command"] as? String {
            return command.count > 80 ? String(command.prefix(80)) + "…" : command
        }
        _ = toolName
        return nil
    }
