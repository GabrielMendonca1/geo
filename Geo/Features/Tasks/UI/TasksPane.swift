...
private struct TaskLane: View, Equatable {
    ...
    let taskListFontSize: CGFloat
    let columnWidth: CGFloat
    let rowSpacing: CGFloat
    let blocksVersion: Int
    let checkboxStateVersion: Int
    let onEdit: (TaskItem) -> Void
    let onComplete: (String) -> Void
    let onMarkPending: (String) -> Void
    let onDelete: (String) -> Void
    let linkedBlockTitle: (TaskItem) -> String?
    let linkedBlockCount: (String) -> Int
    let checkboxes: (TaskItem) -> [BlockCheckbox]
    let hasLinkedBlock: (TaskItem) -> Bool
    let onToggleCheckbox: (String, String, Int) -> Void
    let onOpenBlock: (String) -> Void

    static func == (lhs: TaskLane, rhs: TaskLane) -> Bool {
        lhs.title == rhs.title &&
        lhs.count == rhs.count &&
        lhs.tasks == rhs.tasks &&
        ...
        lhs.taskListFontSize == rhs.taskListFontSize &&
        lhs.columnWidth == rhs.columnWidth &&
        lhs.rowSpacing == rhs.rowSpacing &&
        lhs.blocksVersion == rhs.blocksVersion &&
        lhs.checkboxStateVersion == rhs.checkboxStateVersion
    }
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 10 * scale) {
            ...
            if isCollapsed {
                collapsed.frame(maxHeight: .infinity, alignment: .topLeading)
            } else if tasks.isEmpty {
                empty.frame(maxHeight: .infinity, alignment: .topLeading)
            } else {
                ScrollView {
                    LazyVStack(spacing: rowSpacing) {
                        ForEach(tasks) { task in
                            TaskCard(
                                ...
                                layoutScale: scale,
                                linkedBlockTitle: linkedBlockTitle(task),
                                linkedBlockCount: task.linkedBlockId.map(linkedBlockCount) ?? 0,
                                hasLinkedBlock: hasLinkedBlock(task),
        ...
        .background(
            RoundedRectangle(cornerRadius: 13 * scale)
                .fill(Palette.secondaryBackground.opacity(0.2))
        )
        .overlay(
            RoundedRectangle(cornerRadius: 13 * scale)
    ...
    private var header: some View {
        HStack(spacing: 8 * scale) {
            ...
            Text("\(count)")
                ...
                .background(Capsule().fill(Palette.background.opacity(0.9)))
    ...
    private var collapsed: some View {
        VStack(alignment: .leading, spacing: 4 * scale) {
            Text("Hidden")
                .font(.system(size: fontSize * 0.84, weight: .semibold))
                .foregroundStyle(Palette.tertiaryForeground)
            Text("Tap “Show Completed” in the toolbar to reveal these cards.")
                .font(.system(size: fontSize * 0.78))
                .foregroundStyle(Palette.tertiaryForeground.opacity(0.8))
        }
        .modifier(LanePlaceholder(scale: scale))
    }
...
private struct LanePlaceholder: ViewModifier {
    let scale: CGFloat
    func body(content: Content) -> some View {
        content
            .padding(10 * scale)
            .frame(maxWidth: .infinity, alignment: .leading)
            .background(RoundedRectangle(cornerRadius: 9 * scale).fill(Palette.background.opacity(0.76)))
            .overlay(RoundedRectangle(cornerRadius: 9 * scale).stroke(Palette.border.opacity(0.16), lineWidth: 1))
    }
}
