...
- **Phase 1 — Spine (unblocks all).** ✅ **DONE 2026-06-03** (`Geo/Shared/Infrastructure/Brains/Brains.swift`, `Geo/Tests/BrainsTests.swift`, 11 tests, suite 595/0): `BrainManifest`/brain.json (Codable, iso8601, atomic save/load); `BrainRegistry` (discovers `Brains/*/brain.json`, personal `"essence"` default, sorts essence-first, paths/index resolution, lazy index cache); `BrainIndex` protocol + `DatabaseBrainIndex` wrapping a `DatabaseService` (get / lexical / listByType work for personal AND domain via one impl); `VectorMath` Accelerate cosine + topK. **DEFERRED to Phase 2b** (build storage where it's exercised, no live-DB change before data exists): per-brain `edges`/`node_vec`(BLOB) schema, and `DatabaseBrainIndex.semanticSearch`/`listNeighbors` (currently `semanticSearch` returns `[]`). New Swift files were added to the `Geo`/`GeoTests` targets via the **`xcodeproj` Ruby gem** (available) — not hand-edited pbxproj.
