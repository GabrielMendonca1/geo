...
~/Vault/
  ...
  Captures/YYYY-MM-DD/ OCR markdown (GarimeCapture uploads only the .md to the VM copy, others read)
...
- **GarimeCapture never writes the local vault.** OCR markdown spools to `~/Library/Application Support/Garime/GarimeCapture/spool/` and uploads to `garime:/mnt/garime/Vault/Captures/<day>/`; the images stay on the Mac in `…/GarimeCapture/archive/<day>/` — where `<day>` is the *archiving* day, not the capture day — and are purged 30 days after it, deleting only the artifacts the daemon itself generated. No image is ever uploaded. Writing anywhere under `~/Vault/` is guarded and refused — see `GarimeCapture/README.md`.
...
| `GarimeCapture/` | Daemon: screenshot dir watch + Vision OCR → `.md` to the local spool and image to the local 30-day archive → uploads only the `.md` to `garime:/mnt/garime/Vault/Captures/` (tests: `GarimeCapture/tests/run.sh`) |
