...
~/Vault/
  ...
  Captures/YYYY-MM-DD/ screenshot+OCR pairs (GarimeCapture uploads to the VM copy, others read)
...
- **GarimeCapture never writes the local vault.** Captures spool to `~/Library/Application Support/Garime/GarimeCapture/spool/` and upload to `garime:/mnt/garime/Vault/Captures/<day>/`. Writing anywhere under `~/Vault/` is guarded and refused — see `GarimeCapture/README.md`.
...
| `GarimeCapture/` | Daemon: screenshot dir watch + Vision OCR → local spool → upload to `garime:/mnt/garime/Vault/Captures/` (tests: `GarimeCapture/tests/run.sh`) |
