...
**Inference.** The gateway runs **primary `anthropic/claude-opus-4-8`** on Gabriel's Claude Max sub (first-party OAuth via the Keychain `Claude Code-credentials` token), with **fallback `openai-codex/gpt-5.5`** — so a rare Max-allowance cap-hit drops to Codex instead of hard-failing (verified: opus `400 out-of-extra-usage` → `fallback=True` → gpt-5.5 answers). This needs a one-line **fork** in `~/.hermes/hermes-agent/agent/anthropic_adapter.py` (~L752): the OAuth branch must set `"User-Agent"` (capitalized) so the Anthropic SDK doesn't double-stamp it into `Anthropic/Python …, claude-cli/…` — a polluted UA makes Anthropic bill the request as API/extra-usage instead of the subscription. **Re-apply after every `hermes update`.** The standalone scripts (whatsapp-extractor Opus decide, day/close briefings, geo-context Haiku hook) call Claude directly via the same Max-OAuth token (`model.full`/`model.nano`). `model.default` must stay set (`claude-opus-4-8`) or agent-mode crons fail with an empty-model error. See memory `hermes-model-provider` for the empirical write-up (the wall is Max quota, not auth; mitigated by low use + caching + the codex fallback).
